...
- Session-local evolving race lines generated by real player and opponent traffic, with deterministic lane/bin wear, visible collision-free grooves, bounded dry grip benefits, wet-rut risks, and repeatable Academy/Test Ride isolation
- Mesa Six-Lap's readable dry-to-storm forecast, with one lap authority driving grip, opponent condition skill, equipment wetness, evolving ruts, surface feedback, gradual atmosphere, and HUD/browser warnings
...
- Activity-specific atmospheric grading, fog, lightweight weather, trackside spectators, flags, event-correct sponsor landmarks, and adaptive synthesized music
- Responsive web frontend with a cross-origin-isolated threaded WebAssembly build, fullscreen presentation, browser input capture, and visibility-aware pause/audio behavior
...
2. Add voiced sponsor briefings while retaining the existing readable text, caption, and event-correct physical sponsor treatment.
