...
*Riding Dirty* is a stylized 3D dirt-bike racing tour built with Godot 4.7. Racecraft V28 includes 19 playable events across Quarry Trail, Pine Ridge, and Red Mesa MX; full championship weekends, Custom Tours, and replayable seasons; named multi-bike fields; tactile arcade handling; adaptive touch controls; bike building and tuning; competitive challenges; replay and ghost systems; a ten-lesson Riding Academy; and persistent progression.
...
Open the seven-page settings surface with `F1`, Back/Select, or the touch SETTINGS button. Audio, Ride, Assists, Camera, Graphics, Access, and Input each have a dedicated page. Every numeric and enumerated option can be changed with keyboard, gamepad, or the on-screen minus/plus controls, and long pages keep the selected row in view. The Input page can force touch controls on or off, swap handedness, and tune control size and opacity.

- **Race difficulty:** Relaxed, Casual, Standard, Challenging, and Expert provide five distinct pace, skill, pressure, and Flow profiles in ordinary offline races. The outer modes apply a bounded `-1` or `+1` authored-tier offset while the intermediate modes make smaller physical adjustments, so players can move up without a difficulty cliff. Daily/weekly challenges and Riding Academy grading remain locked to their authored rules, and difficulty is included in the run signature so unlike sessions do not share records.
...
- **Evolving race lines:** Real player and opponent traffic forms visible route-relative grooves during each race. Dry compacted lines offer a small bounded grip/drive benefit; wet or muddy grooves become a readable handling risk. Wear is deterministic per rider and route bin, resets with the run, uses one batched zero-collision mesh, and stays disabled in Academy and Bike Test Ride for repeatable teaching and comparison.
- **Coherent variable weather:** Mesa Six-Lap publishes its complete dry → wind → cloud → rain → storm → drying forecast before the gate. One deterministic lap policy drives player grip, opponent condition skill, roost and surface audio, rider/bike wetness, evolving-rut risk, gradually blended sky/precipitation, HUD warnings, and browser-readable state. Competitive record revision 7 isolates the resulting pace model.
...
- **Race formats:** Circuit, enduro, motocross, elimination, head-to-head rival, endurance, hillclimb, wet race, rhythm attack, freestyle, discovery, daily challenge, weekly challenge, player-authored Custom Tour championships, and Academy sessions.
...
- **Opponent challenge:** Ordinary race fields inherit `0.65` of the active bike and selected Trail, Balanced, or Attack setup's forward-performance gain, capped at a `1.12` opponent multiplier. A starter build stays on the authored baseline while upgrades retain a meaningful player advantage without making the field obsolete. All five difficulty modes remain ordered and measurably distinct; skilled rivals commit to passes and bounded defensive moves, carry stronger section momentum, recover from mistakes, bank Flow, and tactically boost with named audiovisual warnings when they attack nearby. One explicit combined tactical budget caps every positive director, close-attack, and Flow contribution, while final-lap assistance fades and a 2.5-second start-contact immunity protects launches. Deterministic full-course acceptance spans three tracks, five difficulties, full traffic, and strong/elite/max-build benchmarks without granting opponents player-relative speed.
...
- **Pack racecraft:** Player and AI drafting use direction and alignment gates rather than proximity alone. Rivals choose ruts, outside berms, and deterministic skill lines after the launch phase; those choices now form shared evolving grooves that can reward a compacted dry line or punish a deep wet rut. Deliberate dirt slides can roost a close follower, while drive penalties and wobble chances stay bounded.
- **Career and garage:** A six-round championship, persistent weekend state, cash and reputation rewards, bike purchases, race classes, parts, condition and repair, tuning presets that alter physical suspension/brake/preload values, rider number, liveries, and riding kit customization. Eight named build slots let riders preserve and revisit complete bike, kit, class, part, tune, and livery strategies without copying wear or odometer state. Each event retains the latest 12 official run plans alongside its PB and pinned reference for deeper iteration without unbounded save growth.
...
- **Riding Academy:** Ten progressive lessons with live ride metrics, prerequisites, explicit rematches, grading, stars, rewards, and a two-objective live lesson HUD. Surface Reading and Grip teaches hardpack, sand, grass, and mud through the real bike physics; the wider syllabus grades starts, manual shifting, rut rails, controlled slides, pumps, clean landings, skill lines, scrubs, Compose saves, dabs, rejoins, drafting passes, and clean contact management.
- **Player options:** Seven settings pages for Audio, Ride, Assists, Camera, Graphics, Access, and Input; independent Master/Music/Engine/Effects/Commentary/Crowd/Interface levels; persisted Relaxed/Casual/Standard/Challenging/Expert race difficulty; five independently configurable physical assists; Performance/Balanced/Quality visual presets plus render-scale, shadow, effect-density, and weather overrides; Reduced Motion, Reduced Flashes, Reduced Particles, semantic audio captions, and HUD safe-area controls; adaptive multitouch riding and Garage controls with handedness, scale, and opacity options; keyboard/gamepad/mouse rebinding with conflict protection; automatic/manual shifting; metric or imperial units; five riding cameras; FOV, camera shake, deadzone, sensitivity, response curve, haptic strength, text scale, high contrast, and color-safe flag colors.
...
- **Audio identity:** Quarry, Pine, and Mesa use distinct procedural score palettes; standard, weekend, finale, and challenge events adapt through base, drive, tension, and results stems with smooth transitions. Engine, Effects, Commentary, Crowd, and Interface feedback route through independently configurable buses, with important semantic cues receiving readable caption equivalents.
- **Production pass:** Tracks stream behind event briefings, Pine's signature-validated baked dressing and exact terrain index preserve authored density while shortening preparation, and a live route-derived fallback prevents stale bakes from becoming authoritative. Adaptive music synthesis overlaps district loading, the Garage presents the selected bike and rider as a readable hero composition, event-correct sponsor arches and finish/open-area boards carry relationship identity into the physical world without collision, dense dressing is batched and spatially indexed, minimap geometry is cached, release probes are excluded, threaded Web payloads negotiate gzip, and the browser wrapper prioritizes a large readable 16:9 canvas at laptop and high-resolution desktop sizes.
...
- **Sponsor relationships:** Dustline Works rewards race precision, Wildbrush Outpost rewards terrain reading, and Sundown Static rewards expressive lines. Each program advances independently through Prospect, Signed, Factory, and Icon ranks; the Garage previews the selected event's relationship, while the live HUD shows the exact objective, progress, cash, reputation, and Style Token reward. Event transitions carry each program's distinct accent, authored tagline, and rides-to-next-rank; ordinary routes receive a collision-free sponsor arch and finish board, while Freestyle and Discovery use bounded open-area boards and Academy/Test Ride remain commercial-free. Completion uses a sponsor-specific pooled SFX sting. Higher ranks pair modestly harder achievable goals with transparent payouts, and the existing verified contract transaction prevents duplicate rewards.
...
The default gate performs a Godot parser/editor scan, a curated set of high-risk physics, route, persistence, settings, progression, competition, and lifecycle probes, and six representative playable activity smokes. It stops at the first failure.
